...
- **Nothing below the header is painted until the first `render()` has run — `data-booting` (2026-09-12).** `#views` is EMPTY in the markup: every card on this page is built by `render()`, in the main script. The browser paints the parsed body before that script executes, and what it painted was the header, the tab row, and then the **footer riding up directly under the tabs** — "Sign in with Google if you want the same data on your other devices" flashing halfway up the page on every refresh, before the plan appeared and shoved it back down. On a first-ever visit the tab row flashed too, since `render()` is what hides it when there is nothing to tab between. **Five of the eight pages had the fault** (swept 2026-09-12, after this one was fixed): this app, Sprint Velocity, League Night, PAPTrack and the `claude-starter` template. Only Golf Handicap, Flow Metrics and the NY calculator were clean, and they are clean for one reason — their cards are REAL MARKUP inside `<main>`, so first paint is already in the right place. (An early read of this said League Night and PAPTrack were in that group. They are not: it was inferred from how many lines of markup sit inside `<main>`, and their panels are all `hidden` until the first render, so `<main>` measures 0 and the footer lands under the header. Count nothing here — measure the rendered box.) Reported by Charles 2026-09-12. The fix is the arrangement the head script already uses for the theme, the zoom and the pin — a thing settled a frame late is a page that visibly jumps as it opens:
  - **Set in the HEAD script, and OUTSIDE its `try`.** Inside it, a browser that refuses `localStorage` throws on the theme read and the gate never goes on at all — and that is the browser most likely to be slow enough to show the flash.
  - **Released in the SAME TASK as the first render** (right after `booted = true`, before `applyPin`). A frame later and the reveal is itself the flash.
  - **`visibility: hidden`, never `display: none`.** `render()` runs while the gate is on and the charts size themselves off their container; a zero-width container is a zero-width chart that never corrects itself, because nothing re-measures once Chart.js has built. The elements keep their boxes; only the painting is held.
  - **A `DOMContentLoaded` backstop removes it too**, so a script that dies on the way to the release can never leave the page blank. This is not theoretical: `haltForNewerData()` **throws**, and it throws before the release. Its own card is `position: fixed` on `document.body`, outside the gate, so the halt still shows.
  - The header is deliberately NOT covered: it is static markup, already correct at first paint, and holding it back would only make the page slower to show anything at all.
- **`#syncBtn`'s width floor is in em, and that is not a style choice (2026-09-12).** The sync button is the one control in the header whose LABEL changes — "☁️ Sign In to Sync" becomes a first name the moment Firebase answers — so without a floor under its widest state the whole header slides on every refresh. The floor has now gone stale TWICE as a pixel number, for two different reasons. 125px was taken at an older type size and stopped covering the label when the pack moved to a 16px rem ramp on 2026-08-22 (raised to 134 on 2026-08-26). Then 134px turned out to cover only the MOUSE size: `.btn.small` takes `--fs-base` on a coarse pointer, two rules above it, so the same label is 126.98px with a mouse and 148.17px on a phone — and the header still slid ~14px on every phone refresh, under a floor written to stop exactly that. **No single pixel number can cover two type sizes.** `min-width: 10.25em` is measured in the button's own type, so it follows the coarse step and any future ramp without being touched. The figure is this app's own: the widest label needs 9.77em at the mouse size, which is the BINDING case because the 9px of padding is a bigger share of a 13px em than of a 16px one, and 9.26em on touch. League Night answered this first (`c3a8376`); Golf Handicap carried the same 134 and the same fault and carries the same block now, and PAPTrack still carries a pixel pin. **tests.html measures both type sizes against the floor and pins the unit** — the coarse size is reached by putting `--fs-base` on the button, because pointer media cannot be emulated from script and the old test was green in a 1280px frame for the whole life of the bug.
...
- **`sampleState()` is the DEMO, and every tab must show something from it.** The
  ...
  sample annotated a budget cell, a property, a debt and a donation, so the row
  hover text was live on Household and Giving and there was nothing to find on
  Vacations or in a summary year — the trip line being the very row the tooltip
...
- **The grid's hover text is `data-tip` + `wireGridTip()`, never a `title`.**
  ...
  - **A ROW'S NOTE IS THE ROW'S HOVER TEXT** (2026-09-12, in two passes on the
    same day). Five tables hold a note against a row — the summary card,
    Property, What You Owe, a trip's lines, a year of donations — and each drew
    a 📝 beside the name with the words hung on it. The first pass moved those
    words off a native `title` and onto the app's own tip, which fixed the two
    things a title gets wrong here (nothing at all on a phone; gone when a
    `.tablewrap` scrolls sideways under a parked cursor) and left the marker
    as the hover TARGET. That was the remaining fault, and Charles named it:
    *"remove the icon and trigger the tooltip off the whole row like in other
    places."* A 15px emoji is a hard thing to point at, and every other hover
    in this app is a whole cell or a whole line — a grid cell, a month row, a
    balance. So the marker is **gone**, and `rowTip(hint, note)` builds the
    row's `data-tip`: the hint is what the row's title used to say ("Click to
    edit", plus the currency a foreign row reads in), the note is a block of its
    own under it with a blank line between, and a row with nothing to say gets
    no attribute at all. It returns the WHOLE attribute for that last reason.
    - **A row with a tip must not also carry a `title`** — two bubbles over one
      line, a moment apart. That took the native titles off all five rows, and
      off **What You Owe's balance cell**, which is the one cell inside a tipped
      row with something of its own to say ("Stated $X in Jan 2026, paid down to
      here"): it is `rowTip(balTitle, l.note)` now, repeating the note so the
      note reads everywhere along the row rather than everywhere but there.
      A title is still right on a row with NO tip — a goal card, a person, a
      budget row label — which is what the 2026-08-26 audit allows, so the test
      reads the rows that changed rather than grepping the file for `.note`.
    - **A note is often a pasted URL, and a URL is one unbreakable word several
      hundred pixels long.** `#gridTip` had `max-width` and nothing else, so an
      Airbnb share link was drawn straight out of the right-hand side of the
      bubble and the rest of it was simply missing (reported with a screenshot,
      same day). `overflow-wrap: anywhere`, which is what the gathered notes box
      already uses on the same text — `.notelist li > .note-text`.
    - **Nothing changed for a phone, deliberately.** The row is a `[data-edit]`
  ...
  - **The tip's zone is `#views`, wired once**, rather than a class on the grid's
    ...
    `wireView` with it — every tab passes through there, and the noted rows are
    on four of them.
...
- **A result goes STRAIGHT on the tape** (Charles, 2026-09-13: *"automatically
  add results to the tape and get rid of the extra button"*). The submit
  handler calls `addToTape(calcLast, expression, false, tapeKind())` on
  success — the kind is the tape's as it stood before the result joined it,
